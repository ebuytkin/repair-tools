...
def parse_mediainfo(path):
    """Run mediainfo CLI asking only for Format (General) and Width (Video).

    Returns a SimpleNamespace with a `tracks` list containing a General track
    (with .format) and a Video track (with .width as int or None).
    """
    try:
        # Request two simple fields: General Format and Video Width (first video track)
        out = subprocess.run(
            ["mediainfo", "--Output=General;%Format%\nVideo;%Width%", str(path)],
            capture_output=True,
            text=True,
        )
    ...
    # mediainfo should return two lines: format & width (width may be NaN)
    lines = [l.strip() for l in stdout.splitlines() if l.strip()]
    fmt = lines[0] if len(lines) >= 1 else None
    width_raw = lines[1] if len(lines) >= 2 else None

    # sanitize width to integer (remove non-digits)
    width = None
    if width_raw:
        import re

        digits = re.sub(r"[^0-9]", "", width_raw)
        if digits:
            try:
                width = int(digits)
            except Exception:
    ...
    # Build minimal track objects compatible with extract_track_info
    general = SimpleNamespace()
    general.track_type = "General"
    general.file_size = None
    general.file_last_modification_date = None
    general.format = fmt
    general.audio_format_list = None
    general.codecs_video = None
    general.duration = None
    general.other_duration = None

    video = SimpleNamespace()
    video.track_type = "Video"
    video.width = width

    return SimpleNamespace(tracks=[general, video])
...
def convert_to_mp4(input_file, file_name, input_directory, audio_pan, force_16x9=False):
    ...
    output_file_name = f"{(str(file_name).split('.')[0]).replace('_pm', '')}_sc.mp4"
...
def main():
    ...
    if args.output:
        ...
        for path in process_directory(input_dir):
            # Use the mediainfo CLI wrapper instead of pymediainfo
            media_info = parse_mediainfo(path)
